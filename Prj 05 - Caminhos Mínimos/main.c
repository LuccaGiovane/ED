...
void adicionaAresta(GRAFO *g, int u, int v, TIPOPESO peso)
{
    ...
    {
        printf("nao aguento mais grafo meu jesus cristinho");
    }
}
...
void visualizarGrafo(GRAFO* g)
{
    for (int i = 0; i < g->vertices; i++)
    {
        ...
        // Colocar os vértices em um array para ordenação
        ...
        for (int j = 0; j < count - 1; j++)
        {
            for (int k = 0; k < count - j - 1; k++)
            {
                if (verticesOrdenados[k] > verticesOrdenados[k + 1])
                {
                    ...
                    // Trocar os pesos correspondentes
        ...
        // Imprimir os vértices e pesos ordenados
